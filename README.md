...
一個專為護理推車設計的即時雙向翻譯系統，支援語音識別、自動語言偵測、即時翻譯與語音合成功能。系統採用透明螢幕設計，讓正反面的使用者都能看到各自語言的翻譯內容。

## 系統特色

**核心功能**：
- **連續語音識別**：使用 OpenAI Whisper API 進行高精度語音轉文字
- **自動語言偵測**：智能判斷說話者角色（護理師或病患）
...
**場景二：病患回應**
- 病患（越南語）：「Có, đau một chút.」
- 系統顯示：
  - Primary（大字）：「有，有點痛。」（中文）
  - Secondary（小字）：「Có, đau một chút.」
- 系統播放：中文語音
...
**顯示邏輯**：
- 護理師說話時：病患看到大字外語翻譯，護理師看到小字中文原文
- 病患說話時：護理師看到大字中文翻譯，病患看到小字外語原文
