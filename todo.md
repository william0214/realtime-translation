...
**問題描述：**
翻譯階段的 RMS 檢查（`if (rms < 0.055) return;`）會誤判已經被 Whisper 成功識別的句子（例如「開始測試」）為靜音，導致翻譯被跳過。

**根本原因：**
噪音/靜音的判斷應該只在 VAD 階段和 final chunk 產生前處理，而不是在翻譯階段再做一次。如果 Whisper 已經成功識別出文字，就不應該再用 RMS 過濾。
...
**需求：**
- [x] 新增「自動判斷」語言選項
- [ ] 修改網頁標題（頁籤名稱）為「即時翻譯系統」

## ✅ 翻譯功能開關（已完成）

**需求：**
- [x] 檢查翻譯功能開關配置（shared/config.ts）
- [x] 開啟翻譯功能（ENABLE_TRANSLATION: true）
- [x] 測試翻譯功能是否正常運作

## 🧪 多語言翻譯單元測試（進行中）

**需求：**
- [ ] 建立多語言翻譯測試檔案（server/translation.multilang.test.ts）
- [ ] 測試越南語翻譯
- [ ] 測試印尼語翻譯
- [ ] 測試菲律賓語翻譯
- [ ] 測試英文翻譯
...
**需求：**
- [x] 設計資料庫 schema（conversations, translations, summaries）
- [x] 實作對話摘要生成 API（使用 LLM）
- [x] 實作後台管理介面
  - [x] 對話列表頁面
  - [x] 對話詳情頁面（顯示完整對話 + 摘要）
  - [x] 摘要生成按鈕
