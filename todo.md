...
## 對話框旋轉方向修正
- [x] 台灣人對話框旋轉 180 度（朝上給對方看）
- [x] 外國人對話框正常朝下（不旋轉）
- [ ] 測試修正後的效果

## 後端切換功能（Node.js ↔ Go）
- [x] 設計隱藏的切換按鈕（三次點擊標題）
- [x] 實作 localStorage 儲存後端選擇
- [x] 實作 Node.js 後端 API 呼叫邏輯
- [x] 實作 Go 後端 API 呼叫邏輯（REST）
- [x] 加入切換提示訊息
- [x] 在標題顯示目前後端（Node.js 或 Go）
- [ ] 測試兩種後端的效果

## VAD 噪音過濾優化（緊急）
- [x] 提高 RMS 閖值過濾背景噪音（從 0.04 提高到 0.08）
